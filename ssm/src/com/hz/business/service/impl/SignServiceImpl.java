...
public class SignServiceImpl implements SignService {
	...
	public List<Sign> findUserSign(String groupId, String userId) {
		SignExample example = new SignExample();
		
		SignExample.Criteria criteria = example.createCriteria();
		criteria.andSignPersonIdEqualTo(userId).andTypeEqualTo(Constants.SIGN_TYPE_SUCCESS);
		
		if(StringUtil.isNotBlank(groupId)){
			criteria.andGroupIdEqualTo(groupId);
		}
		List<Sign> list = manager.getSignMapper().selectByExample(example);
