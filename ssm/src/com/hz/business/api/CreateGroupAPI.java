...
//			}
		...
		public void updateGroup(HttpServletRequest request ,HttpServletResponse response,@ModelAttribute("baseModel") BaseModel baseModel,Model model) throws IOException{
			
			JSONObject obj = new JSONObject();
			if(StringUtils.isBlank(baseModel.getGroupId())){
				obj.put("success", false);
				obj.put("msg", "群组ID不能为空");
				obj.put("status", "3");
				response.getWriter().write(new Gson().toJson(obj));
				return;
			}
			
			boolean success = groupService.updateGroup( Integer.valueOf(baseModel.getGroupId()) , baseModel.getGroupName(), baseModel.getGroupDesc(), baseModel.getHaveOpenSign() );
